...
import collections
from pcefDemoConst import MZ_ROOT, PCEF_PORTS, SOAP_PORT

from pysimplesoap.client import SoapClient
from UE import UE
from PCEF import PCEF
from PCC import diameterErrors
import PCC
...
def initialize():
    pcefs.clear()
    pcefs[1] = PCEF(1, "PCEF v1", MZ_ROOT, PCEF_PORTS[1])
    pcefs[2] = PCEF(2, "PCEF v2", MZ_ROOT, PCEF_PORTS[2])
...
def getSOAPClient():
    return SoapClient(wsdl=MZ_ROOT + ":" + SOAP_PORT + "/pcrfWSHandler?WSDL",action="",trace=False)
...
def monitor_ue_session(identity=None, sessionid = None):
    ...
    if sessionid not in sessions or sessions[sessionid].identity != identity:
        abort(404)
    buckets = getBuckets(identity)
...
def pcef_rule(ruleid = None, pcefid=None):
    if ruleid not in pcefs[pcefid].staticRules:
        abort(404)
...
def pcef_delete_session(pcefid=None, sessionid=None):
    ...
    if sessionid not in pcefs[pcefid].sessions:
...
def reset_bdh(identity=None):
    if identity not in all_users and identity != "Family":
        abort(404)
