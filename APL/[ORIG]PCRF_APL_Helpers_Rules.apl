...
void processRules(rc rc) {
    ...
    // Get rules
    list<PCC_Rule> rules = getRules(rc.bdh, rc.timestamp, pccRuleDeactMap);
        ...
        && bdh.Subscriber != null) {
            ...
            while (k < listSize(bdh.Sessions)) {
                ...
                if (listSize(parts) == 2) {
                    ...
                    // Get rules
                    list<PCC_Rule> rules = getRules(bdhMap, dateCreateNow(), pccRuleDeactTimeMap);
 ...
 * 
 */
list<PCC_Rule> getRules(map<string, BucketDataHolder> bdhMap, date timestamp, map<int, date> ruleDeactTimeMap) {
    ...
    while (i < listSize(mapping) && !stopFallthrough) {
        ...
        while (j < listSize(items)) {
            ...
            list<string> arguments = listCreate(string, (string)product, enforcement);
